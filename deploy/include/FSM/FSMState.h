...
public:
    ...
    void pre_run()
    {
        if(remote_joystick && remote_joystick->isActive()) {
            // When remote joystick is active, skip lowstate->update() which
            // would extract zeros from empty wireless_remote and fight with
            // our UDP values due to Axis smoothing (smooth=0.03, threshold=0.5).
            // lowstate->update() only processes joystick — motor/IMU data in
            // msg_ is updated by DDS callbacks and accessed directly.
            remote_joystick->applyTo(lowstate->joystick);
        } else {
            lowstate->update();
        }
    ...
    static std::shared_ptr<NetworkJoystick> remote_joystick;
};