...
std::shared_ptr<NetworkJoystick> FSMState::remote_joystick = nullptr;
...
int main(int argc, char** argv)
{
    ...
    int rj_port = vm["remote-joystick"].as<int>();
    if (rj_port > 0) {
        FSMState::remote_joystick = std::make_shared<NetworkJoystick>();
        if (!FSMState::remote_joystick->start(rj_port)) {
            spdlog::critical("Failed to start NetworkJoystick on port {}", rj_port);
            exit(-1);
        }
    }
